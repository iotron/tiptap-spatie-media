...
namespace Iotronlab\TiptapSpatieMedia\Support;
...
trait HasTiptapSpatieMediaActionFormSchema
{
     ...
     */
    protected static function OnUpdate(BaseFileUpload $component, TemporaryUploadedFile $file, ?Model $record): mixed
    {
        ...
        $mediaInstance = $record->addMedia($file)
            ...
            ->toMediaCollection(TiptapSpatieMedia::mediaCollection($record));
     ...
     */
    protected static function OnCreate(BaseFileUpload $component, TemporaryUploadedFile $file, ?Model $record): mixed
    {
        $filename = $component->shouldPreserveFilenames() ? pathinfo($file->getClientOriginalName(), PATHINFO_FILENAME) : Str::uuid();
